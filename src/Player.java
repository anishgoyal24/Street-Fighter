...
public class Player implements IGameConstants, IPlayerStates {
    ...
    private BufferedImage specialPower[] = new BufferedImage[3];
    private BufferedImage powerEffects[] = new BufferedImage[3];
    ...
    private void loadSpecialPower(){
        specialPower[0] = image.getSubimage(8, 663, 57, 75);
        specialPower[1] = image.getSubimage(91, 664, 54, 75);
        specialPower[2] = image.getSubimage(162, 665, 66, 75);

    }

    private void loadPowerEffects(){
        powerEffects[0] = image.getSubimage(236, 661, 42, 52);
        powerEffects[1] = image.getSubimage(287, 673, 42, 37);
        powerEffects[2] = image.getSubimage(335, 677, 42, 30);
    }
    ...
    int specialPowerIndex = 0;
    int specialPowerDelay = 0;

    public void specialPower(Graphics graphics){
        if (specialPowerIndex >= 3){
            specialPowerIndex = 0;
            state = IDLE;
            counter = 0;
        }
        graphics.drawImage(specialPower[specialPowerIndex], x, y, w, h, null);
        if (specialPowerDelay >= 3){
            specialPowerDelay = 0;
            specialPowerIndex++;
        }
        specialPowerDelay++;
